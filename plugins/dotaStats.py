...
def quick_game_details(res):
    ...
    gold_str = ("**Gold:** +" + str(abs(gold)) + (" Radiant" if gold >= 0 else " Dire")) if not gold is None else "N/A"
    xp_str = ("**Experience:** +" + str(abs(xp)) + (" Radiant" if xp >= 0 else " Dire")) if not xp is None else "N/A"
    return(length + "\n" + score + "\n" + gold_str + "\n" + xp_str)
