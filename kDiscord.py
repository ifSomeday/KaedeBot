...
def discBot(kstQ, dscQ, factoryQ, draftEvent):
    ...
    async def create_lobby(*args, **kwargs):
        if('msg' in kwargs):
            ...
            lobby_name = "WRICE lobby DEADBEEFDEADBEEF"
    ...
    async def on_message_delete(message):
        if(cfg.checkMessage("deletion", message) and (not message.author.id == header.MY_DISC_ID)):
            if(deleteFilter(message.content)):
                return
            if(message.server.id == '308515912653340682' and not (message.author.id == '171840790803382272' or message.author.id == "117446235715010569")):
                return
